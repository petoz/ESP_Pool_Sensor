...
void setup() {
  ...
  if (!resetReason.equals("Deep-Sleep Wake") && drd->detectDoubleReset()) {
    ...
    loadSettings();
    connectWiFiAndMQTT();
    checkForMQTTCommands(2000);     // allow incoming interval update
    sendData();
    goToSleep();
  }
}
...
void startConfigPortal() {
  WiFiManagerParameter p_mqtt(TOPIC_TEMP,     "MQTT Server",   mqtt_server, SERVER_SIZE);
  ...
  if (!wm.startConfigPortal("PoolSensorAP")) ESP.restart();
